...
class Parser:
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected int, float, identifier, '+', '-', '(' or 'nahi'",
                )
            )
    ...
    def expr(self):
        ...
        node = res.register(
            self.bin_op(self.comp_expr, ((TT_KEYWORD, "dono"), (TT_KEYWORD, "ya")))
        )
        ...
        if res.error:
            return res.failure(
                InvalidSyntaxError(
                    self.current_tok.pos_start,
                    self.current_tok.pos_end,
                    "Expected 'ye', int, float, identifier, '+', '-', '(' or 'nahi'",
                )
            )
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        elif node.op_tok.matches(TT_KEYWORD, "dono"):
